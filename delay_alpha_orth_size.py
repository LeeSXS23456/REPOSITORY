...
def orthoganalize_delay_with_size(delay_dict, mcp_dict):
    ...
    for date in delay_dict.keys():
        ...
        ortho_delay_dict[date] = delay_subset
    
    logger.info("正交化完成")
    return ortho_delay_dict
...
if __name__ == "__main__":
    ...
    if sample_dates:
        ...
        with pd.ExcelWriter(f"{desdir}/正交化延迟指标_2024_2026_sample.xlsx") as writer:
            for date in sample_dates:
                if date in ortho_delay_dict:
                    sheet_name = date.strftime('%Y-%m-%d')
                    ortho_delay_dict[date].to_excel(writer, sheet_name=sheet_name)
        logger.info(f"保存正交化示例数据到 {desdir}/正交化延迟指标_2024_2026_sample.xlsx")
