...
# ===== 配置 =====
barra_dir = r'E:\SJTU\intern\gtht\barra\data_base\barra_data\whole_mkt'
ret_dir   = r'E:\SJTU\intern\gtht\barra\data_base\stk_ret'
mcp_dir   = r'E:\SJTU\intern\gtht\barra\data_base\stk_mcp'
desdir    = r'E:\SJTU\intern\gtht\barra\result\自算因子收益率'
rf_daily  = 0.015 / 252   # 年化无风险利率日度化
weight_type = 'equal'  # 'sqrt_cap' / 'cap' / 'equal'
...
prefix = f'{desdir}/whole_mkt_{weight_type}'
df_facret.to_excel(f'{prefix}_因子收益率截止至{ret_dates[-1]}.xlsx')

df_speret = pd.concat(speret_lst, axis=1).T
df_speret.index = ret_dates
df_speret.to_excel(f'{prefix}_特质收益率截止至{ret_dates[-1]}.xlsx')

pd.DataFrame.from_dict(R2_dict, orient='index', columns=['R2']).to_excel(
    f'{prefix}_截面回归R方截止至{ret_dates[-1]}.xlsx', index_label='日期')
...
plt.tight_layout()
plt.savefig(f'{prefix}_累计因子收益净值_截止{ret_dates[-1]}.png')
plt.show()
